...
description: Use this skill when a feature branch PR has merge conflicts with the base branch (main or master) and needs to be resolved and pushed. For frontend UI conflicts, preserve shadcn/ui structure and conventions.
---
# Objective
...
## Frontend UI Conflict Rules
- For conflicts in `src/components/ui/`, preserve shadcn component API shape and variant patterns.
- For conflicts in feature UI files, keep feature-local compositions in `src/features/<feature-name>/components/`.
- Use `.github/references/shadcn-ui-components.md` when deciding between conflicting UI patterns.
...
9. Run project validation (for this repo, compile before push):
    - For frontend repo: `npm run lint` and `npm run build`
