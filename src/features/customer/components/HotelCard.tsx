  ...
  return (
    <Card className="group overflow-hidden bg-card/50 border-white/5 hover:border-white/20 transition-all duration-500 glass hover:shadow-2xl hover:-translate-y-1">
      ...
      <CardContent className="p-5 pb-2 relative">
        ...
        <Badge
          variant="secondary"
          className="absolute -top-4 left-5 bg-primary/20 text-primary hover:bg-primary/30 border-primary/20 backdrop-blur-lg backdrop-filter"
        >
          Top Rated
        </Badge>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {hotel.amenities.map((amenity) => (
            <div
              key={amenity}
              className="flex items-center text-[11px] font-medium px-2 py-1 rounded-md bg-secondary/40 text-secondary-foreground border border-white/5"
            >
      ...
      <CardFooter className="p-5 pt-0 mt-auto flex items-end justify-between border-t border-white/5">
        ...
        <Button
          size="sm"
          className="rounded-xl px-5 shadow-primary/20 shadow-lg group-hover:bg-primary/90 transition-all font-bold"
        >
          Book Now
        </Button>
      </CardFooter>
